/*
  Random input test. Inputs are from csv files. The sum of abs of coefs is < 1.0.
*/
#include <stdio.h>
#include <string.h>
#include "mac_fixed.h"
...
int main(void)
{
	...
	sprintf(r_filename, "./input_test_cases/%d_in_%d_c_%d_case_maxcsum.csv", DATA_LEN, COEF_LEN, TEST_CASE_NUM);
	sprintf(w_filename, "./output/%d_in_%d_c_%d_case_maxcsum.csv", DATA_LEN, COEF_LEN, TEST_CASE_NUM);
